...
class GPEIOptChooser:
    ...
    def grad_optimize_ei(self, cand, comp, pend, vals, compute_grad=True):
        if pend.shape[0] == 0:
            ...
            # Apply covariance function
            # Used to be: grad_cross = np.squeeze(cand_cross_grad). This leads to problems
            # if the last TWO dimensions are 1 (i.e. there is just one dimensions
            # to optimize over). The following is robust in this case:
            grad_cross_squeezed = np.squeeze(cand_cross_grad)
            if grad_cross_squeezed.ndim == 1:
              grad_cross = grad_cross_squeezed.reshape((grad_cross_squeezed.shape[0], 1))
            else:
              grad_cross = grad_cross_squeezed
