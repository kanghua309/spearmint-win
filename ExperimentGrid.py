...
class ExperimentGrid:
    ...
    def __del__(self):
        self._save_jobs()
        if self.locking:
          if self.locker.unlock(self.jobs_pkl_lock):
    ...
    def _save_jobs(self):
        ...
        if os.path.exists(self.jobs_pkl):
            fh   = open(self.jobs_pkl, 'r')
            stored_jobs = cPickle.load(fh)
            fh.close()
            if (self._safe_compare(self.grid, stored_jobs['grid'])
                and self.vmap == stored_jobs['vmap']
                and np.all(self.status == stored_jobs['status'])
                and self._safe_compare(self.values, stored_jobs['values'])
                and self._safe_compare(self.durs, stored_jobs['durs'])
                and self._safe_compare(self.sgeids, stored_jobs['sgeids'])):
                write = False
        if write:
